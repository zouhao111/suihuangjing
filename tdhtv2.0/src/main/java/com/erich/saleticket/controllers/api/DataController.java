...
public class DataController extends BaseController{
	...
	public String DataController(ViewCompanyconfig companyconfigs, HttpServletRequest request, HttpServletResponse response)   {
		...
		if(data_ls!=null){
			if(data_ls.getLs_sub_ztraffic()==null){
		...
		Pollutant pollutants=dataBiz.selectByPollutant2_sum(rid);//污染物排放总量
		...
		waterquality.setRealConductivity(fixed(waterquality.getRealConductivity(), 0));//电导率
		waterquality.setLastDayMaxConductivity(fixed(waterquality.getLastDayMaxConductivity(), 0));//
		waterquality.setLastDayMinConductivity(fixed(waterquality.getLastDayMinConductivity(), 0));
		...
		waterquality.setRealTurbidity(fixed(waterquality.getRealTurbidity(), 1));
		waterquality.setLastDayMaxTurbidity(fixed(waterquality.getLastDayMaxTurbidity(), 1));
		waterquality.setLastDayMinTurbidity(fixed(waterquality.getLastDayMinTurbidity(), 1));
		waterquality.setRealDo(fixed(waterquality.getRealDo(), 2));
		waterquality.setLastDayMaxDo(fixed(waterquality.getLastDayMaxDo(), 2));
		waterquality.setLastDayMinDo(fixed(waterquality.getLastDayMinDo(), 2));
		...
		//}
			...
			pollutantss.setRealcu(fixed(pollutantss.getRealcu(), 3));//总铜
			pollutantss.setMaxcu(fixed(pollutantss.getMaxcu(), 3));
			pollutantss.setMincu(fixed(pollutantss.getMincu(), 3));
			pollutantss.setRealni(fixed(pollutantss.getRealni(), 3));//总镍
			pollutantss.setMaxni(fixed(pollutantss.getMaxni(), 3));
			pollutantss.setMinni(fixed(pollutantss.getMinni(), 3));
			pollutantss.setRealFlu(fixed(pollutantss.getRealFlu(), 3));//氟化物
			pollutantss.setMaxFlu(fixed(pollutantss.getMaxFlu(), 3));
			pollutantss.setMinFlu(fixed(pollutantss.getMinFlu(), 3));
		...
		if(pollutants !=null){
			...
			if(pollutants.getPrussiate()==null){
				pollutants.setPrussiate(0.0);
			}else{
				pollutants.setPrussiate(fixed(pollutants.getPrussiate(), 4)); //氰化物
			}
			if(pollutants.getFlu()==null){
				pollutants.setFlu(0.0);
			}else{
				pollutants.setFlu(fixed(pollutants.getFlu(), 3)); //氟化物
			}
...
*/	
	...
	@Get("dataListPollutant")
	@Post("dataListPollutant")
	public String DataControllerPollutant(ViewCompanyconfig companyconfigs, HttpServletRequest request, HttpServletResponse response){
		MobileData<Object> result = new MobileData<Object>(ApiResultcode._0);
		...
		if(data_ls!=null){
			data.setIs_sub_ztraffic(data_ls.getIs_sub_ztraffic());
			}
		if(data_id!=null){
			data.setId_sub_ztraffic(data_id.getId_sub_ztraffic());
		...
		waterquality.setLastDayMaxPh(fixed(waterquality.getLastDayMaxPh(), 1));//ph最大
		waterquality.setLastDayMinPh(fixed(waterquality.getLastDayMinPh(), 1));//ph最小
		waterquality.setRealConductivity(Math.floor(waterquality.getRealConductivity()));//电导率
		waterquality.setLastDayMaxConductivity(fixed(waterquality.getLastDayMaxConductivity(), 0));//
		waterquality.setLastDayMinConductivity(fixed(waterquality.getLastDayMinConductivity(), 0));
		waterquality.setRealCod(fixed(waterquality.getRealCod(), 0));//COD
		waterquality.setLastDayMaxCod(fixed(waterquality.getLastDayMaxCod(), 0));
