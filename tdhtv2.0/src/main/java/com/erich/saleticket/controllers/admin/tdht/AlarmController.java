...
public class AlarmController extends BaseController {
	...
	private DataBiz dataBiz;
			...
			public String alarmList(Alarm alarm, HttpServletRequest request, HttpServletResponse response) throws ParseException {
				 ...
				 * 根据rid查询该公司的污染物种类和限值
				 */
				List<AlarmType>  alarmType=alarmBiz.selectByCriteriaType(rid);
				...
				String eTime = (new SimpleDateFormat("yyyy-MM-dd HH:mm")).format(cal.getTime()); 
				String sTime=(new SimpleDateFormat("yyyy-MM-dd HH:mm")).format(viewCompanyconfig.getStratTime());
				int type=5;
				 List<String> times=Time_sub(sTime, eTime,type);
				 
				if(times.size()==4){
					alarm1.setStartingTime(times.get(0));
					alarm1.setEndTime(times.get(1));
					alarm1.setRid(rid);
					
					
					alarm2.setStartingTime(times.get(2));
					alarm2.setEndTime(times.get(3));
					...
					alarm3.setStartingTime(times.get(4));
					alarm3.setEndTime(times.get(5));
					alarm3.setRid(rid);
				
				
					alarm4.setStartingTime(times.get(6));
					alarm4.setEndTime(times.get(7));
					alarm4.setRid(rid);
				   datasize=4;
				}
			*/
			...
			*/
			...
			public String historyList(Alarm alarm, HttpServletRequest request, HttpServletResponse response) throws ParseException {
				...
				List<Alarm> codAlarm=new ArrayList<Alarm>();
				 int pagenum=0;
				 Pager pagers = new Pager();
				if(alarmsss.isHave()){
				if(waterType.equals("ph")){
					
					alarmsss.getPager().setRowCount(alarmBiz.selectCountPh(alarmsss));
					request.setAttribute("pager", alarmsss.getPager());
					int size=alarmsss.getPager().getRowCount();
					if(size==0){
						alarmsss.setPager(null);
					}
					 historys = alarmBiz.selectByCriteriaPh(alarmsss);
				}
				if(waterType.equals("cod")){
					alarmsss.getPager().setRowCount(alarmBiz.selectCountCod(alarmsss));
					request.setAttribute("pager", alarmsss.getPager());
					int size=alarmsss.getPager().getRowCount();
					if(size==0){
						alarmsss.setPager(null);
					}
					 historys = alarmBiz.selectByCriteriaCod(alarmsss);
				}
				if(waterType.equals("mlss")){
					alarmsss.getPager().setRowCount(alarmBiz.selectCountMlss(alarmsss));
					request.setAttribute("pager", alarmsss.getPager());
					int size=alarmsss.getPager().getRowCount();
					if(size==0){
						alarmsss.setPager(null);
					}
					 historys = alarmBiz.selectByCriteriaMlss(alarmsss);
				}
				if(waterType.equals("pho")){
					alarmsss.getPager().setRowCount(alarmBiz.selectCountPho(alarmsss));
					request.setAttribute("pager", alarmsss.getPager());
					int size=alarmsss.getPager().getRowCount();
					if(size==0){
						alarmsss.setPager(null);
					}
					 historys = alarmBiz.selectByCriteriaPho(alarmsss);
				}
				if(waterType.equals("ztraffic")){
					alarmsss.getPager().setRowCount(alarmBiz.selectCountZtr(alarmsss));
					request.setAttribute("pager", alarmsss.getPager());
					int size=alarmsss.getPager().getRowCount();
					if(size==0){
						alarmsss.setPager(null);
					}
					 historys = alarmBiz.selectByCriteriaZtr(alarmsss);
				}
				if(waterType.equals("cr6")){
					alarmsss.getPager().setRowCount(alarmBiz.selectCountcr6(alarmsss));
					request.setAttribute("pager", alarmsss.getPager());
					int size=alarmsss.getPager().getRowCount();
					if(size==0){
						alarmsss.setPager(null);
					}
					 historys = alarmBiz.selectByCriteriacr6(alarmsss);
				}
				if(waterType.equals("vol")){
					alarmsss.getPager().setRowCount(alarmBiz.selectCountVol(alarmsss));
					request.setAttribute("pager", alarmsss.getPager());
					int size=alarmsss.getPager().getRowCount();
					if(size==0){
						alarmsss.setPager(null);
					}
					 historys = alarmBiz.selectByCriteriaVol(alarmsss);
				}
				if(waterType.equals("cu")){
					alarmsss.getPager().setRowCount(alarmBiz.selectCountcu(alarmsss));
					request.setAttribute("pager", alarmsss.getPager());
					int size=alarmsss.getPager().getRowCount();
					if(size==0){
						alarmsss.setPager(null);
					}
					 historys = alarmBiz.selectByCriteriacu(alarmsss);
				}
				if(waterType.equals("ni")){
					alarmsss.getPager().setRowCount(alarmBiz.selectCountni(alarmsss));
					request.setAttribute("pager", alarmsss.getPager());
					int size=alarmsss.getPager().getRowCount();
					if(size==0){
						alarmsss.setPager(null);
					}
					 historys = alarmBiz.selectByCriteriani(alarmsss);
				}
				if(waterType.equals("zn")){
					alarmsss.getPager().setRowCount(alarmBiz.selectCountzn(alarmsss));
					request.setAttribute("pager", alarmsss.getPager());
					int size=alarmsss.getPager().getRowCount();
					if(size==0){
						alarmsss.setPager(null);
					}
					 historys = alarmBiz.selectByCriteriazn(alarmsss);
				}
				if(waterType.equals("Tur")){
					alarmsss.getPager().setRowCount(alarmBiz.selectCountTur(alarmsss));
					request.setAttribute("pager", alarmsss.getPager());
					int size=alarmsss.getPager().getRowCount();
					if(size==0){
						alarmsss.setPager(null);
					}
					 historys = alarmBiz.selectByCriteriaTur(alarmsss);
				}
				...
				request.setAttribute("viewCompanyconfig", viewCompanyconfig);
				request.setAttribute("historys", codAlarm);
				request.setAttribute("rid", rid);
				
				request.setAttribute("waterType", waterType);
				return "alarm/historyList";
			}
			...
			public String warninghistoryList(Alarm alarm, HttpServletRequest request, HttpServletResponse response) throws ParseException {
				...
				for(int i=0;i<alarmType.size();i++){
					...
					if(alarmType.get(i).getType().equals("总镍")){
						...
						alarmni.setMin_early(niMax);
						//alarmni.setMax_early(niMaxs);
						//alarmni.setMinValue(niMax);
						alarmni.setMaxValue(niMaxs);
						alarmni.setCompanyName(viewCompanyconfig.getCompanyName());
						alarmni.setRid(rid);
						boolean have=(alarmType.get(i).getHave())==1?true:false;
						alarmni.setHave(have);
					...
					if(alarmType.get(i).getType().equals("总锌")){
						...
						alarmzn.setMin_early(znMax);
						alarmzn.setMax_early(znMaxs);
						//alarmzn.setMinValue(znMax);
						//alarmzn.setMaxValue(znMaxs);
						alarmzn.setCompanyName(viewCompanyconfig.getCompanyName());
						alarmzn.setRid(rid);
						boolean have=(alarmType.get(i).getHave())==1?true:false;
						alarmzn.setHave(have);
				...
				return "alarm/warninghistoryList";
			}
