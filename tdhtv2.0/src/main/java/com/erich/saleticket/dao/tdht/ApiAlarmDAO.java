...
public interface ApiAlarmDAO {
	...
	@SQL("select * from alarm_limit where rid=:1")
	 public List<AlarmType>  selectByCriteria_type(Long rid);
	
	
	@SQL("select * from tabpollutant  where rid=:1.rid  " +
			"#if(!empty(:1.maxValue)){and :1.alarm_name>:1.maxValue } " +
			"#if(!empty(:1.min_early)){and :1.alarm_name>:1.min_early } " +
			"#if(!empty(:1.max_early)){and :1.alarm_name<:1.max_early } " +
			" #if(!empty(:1.startingTime)){ and strTime>= '##(:1.startingTime)' }"+
			" #if(!empty(:1.endTime)){and strTime<= '##(:1.endTime)' }"+
			" #if(!empty(:1.dateTypes)){and strTime like '##(:1.dateTypes)%' }"+
			" #if(!empty(:1.zeroTime)){ and strTime>= '##(:1.zeroTime)' }"+
			...
			" #if(!empty( :1.limit)){  limit :1.limit} " +
			"  #if(!empty( :2.pageOffset)){ limit :2.pageOffset, :2.pageSize}" )
	 public List<Pollutant>  selectByCriteria_all(Alarm alarm,Pager pager);
	
	@SQL("SELECT * from tabpollutant,(select rid,max(syTime)as maxSytime from tabpollutant  where rid=:1.rid " +
			"#if(!empty(:1.startingTime)){ and strTime>= '##(:1.startingTime)' }"+
			"#if(!empty(:1.endTime)){and strTime<= '##(:1.endTime)' }"+
			"#if(!empty(:1.maxValue)){and ##(:1.alarm_name)>:1.maxValue} " +
			"#if(!empty(:1.min_early)){and ##(:1.alarm_name)>:1.min_early} " +
			"#if(!empty(:1.max_early)){and ##(:1.alarm_name)<:1.max_early} " +
			") as tabpoll where  tabpollutant.syTime=tabpoll.maxSytime and tabpollutant.rid=tabpoll.rid")
	 public List<Pollutant>  selectByCriteria_allAPI(Alarm alarm,Pager pager);
	...
	@SQL("SELECT  rid,strTime as strTime,tabVoltage as realvoltage ,tabTurbidity as realTurbidity from sourcemidtabindustrialdrainage,(select rid as rid1,max(syTime)as maxSytime from sourcemidtabindustrialdrainage  where rid=:1.rid  " +
			" #if(!empty(:1.startingTime)){ and strTime>= '##(:1.startingTime)' }"+
			" #if(!empty(:1.endTime)){and strTime<= '##(:1.endTime)' }"+
			"#if(!empty(:1.maxValue)){and  ##(:1.alarm_name)>:1.maxValue} " +
			"#if(!empty(:1.minValue)){and  ##(:1.alarm_name)<:1.minValue} " +
			"#if(!empty(:1.min_early)){and  ##(:1.alarm_name)>:1.min_early} " +
			"#if(!empty(:1.max_early)){and  ##(:1.alarm_name)<:1.max_early} " +
			") as tabpoll where  sourcemidtabindustrialdrainage.syTime=tabpoll.maxSytime and sourcemidtabindustrialdrainage.rid=tabpoll.rid1")
	 public List<Pollutant>  selectByCriteria_sourAllAPI(Alarm alarm,Pager pager);
	...
	@SQL("SELECT * from tabpollutant,(select rid,max(syTime)as maxSytime from tabpollutant  where rid=:1.rid  " +
			...
			"#if(!empty(:1.max_early)){and realcod<:1.max_early } "+ 
			") as tabpoll where  tabpollutant.syTime=tabpoll.maxSytime and tabpollutant.rid=tabpoll.rid")
	 public List<Pollutant>  selectByCriteria_codAPI(Alarm alarm,Pager pager);
	...
	@SQL("SELECT * from tabpollutant,(select rid,max(syTime)as maxSytime from tabpollutant  where rid=:1.rid " +
			...
			"#if(!empty(:1.max_early)){and realnh4<:1.max_early} " +
			") as tabpoll where  tabpollutant.syTime=tabpoll.maxSytime and tabpollutant.rid=tabpoll.rid")
	 public List<Pollutant>  selectByCriteria_nh4API(Alarm alarm,Pager pager);
	...
	@SQL("SELECT * from tabpollutant,(select rid,max(syTime)as maxSytime from tabpollutant  where rid=:1.rid  " +
			...
			"#if(!empty(:1.max_early)){and realmlss<:1.max_early} " +
			") as tabpoll where  tabpollutant.syTime=tabpoll.maxSytime and tabpollutant.rid=tabpoll.rid")
	 public List<Pollutant>  selectByCriteria_mlssAPI(Alarm alarm,Pager pager);
	...
	@SQL("SELECT * from tabpollutant,(select rid,max(syTime)as maxSytime from tabpollutant  where rid=:1.rid  " +
			...
			"#if(!empty(:1.max_early)){and realphosphor<:1.max_early}  " +
			") as tabpoll where  tabpollutant.syTime=tabpoll.maxSytime and tabpollutant.rid=tabpoll.rid")
	 public List<Pollutant>  selectByCriteria_phoAPI(Alarm alarm,Pager pager);
	...
	@SQL("SELECT * from tabpollutant,(select rid,max(syTime)as maxSytime from tabpollutant  where rid=:1.rid  " +
			...
			"#if(!empty(:1.max_early)){and realcr6<:1.max_early} " +
			") as tabpoll where  tabpollutant.syTime=tabpoll.maxSytime and tabpollutant.rid=tabpoll.rid")
	 public List<Pollutant>  selectByCriteria_cr6API(Alarm alarm,Pager pager);
	...
	@SQL("SELECT rid,strTime,tabph as realph from midtabindustrialdrainage,(select rid as rid1,max(syTime)as maxSytime from midtabindustrialdrainage  where rid=:1.rid  " +
			" #if(!empty(:1.startingTime)){ and strTime>= '##(:1.startingTime)' }"+
			" #if(!empty(:1.endTime)){and strTime<= '##(:1.endTime)' }"+
			"and tabPh>0 and tabPh<14 " +
			"#if(!empty(:1.maxValue)){and (tabph>:1.maxValue or tabph<:1.minValue)}" +
			"#if(!empty(:1.min_early)){and ((tabph>:1.min_earlys and tabph<:1.min_early) or (tabph>:1.max_early and tabph<:1.max_earlys))} " +
			") as tabpoll where  midtabindustrialdrainage.syTime=tabpoll.maxSytime and midtabindustrialdrainage.rid=tabpoll.rid1")
	 public List<Pollutant>  selectByCriteria_phAPI(Alarm alarm,Pager pager);
	...
	@SQL("SELECT rid,Time as strTime,a1 as realztraffic  from ztrafficusub_total_4,(select rid as rid1,max(syTime)as maxSytime from ztrafficusub_total_4  where rid=:1.rid  " +
			" #if(!empty(:1.startingTime)){ and Time>= '##(:1.startingTime)' }"+
			" #if(!empty(:1.endTime)){and Time<= '##(:1.endTime)' }"+
			"#if(!empty(:1.maxValue)){and a1>:1.maxValue} " +
			"#if(!empty(:1.min_early)){and a1>:1.min_early} " +
			"#if(!empty(:1.max_early)){and a1<:1.max_early} " +
			") as tabpoll where  ztrafficusub_total_4.syTime=tabpoll.maxSytime and ztrafficusub_total_4.rid=tabpoll.rid1")
	 public List<Pollutant>  selectByCriteria_ztrAPI(Alarm alarm,Pager pager);
	...
	@SQL("SELECT  rid,strTime as strTime,tabVoltage as realvoltage from sourcemidtabindustrialdrainage,(select rid as rid1,max(syTime)as maxSytime from sourcemidtabindustrialdrainage  where rid=:1.rid  " +
			" #if(!empty(:1.startingTime)){ and strTime>= '##(:1.startingTime)' }"+
			" #if(!empty(:1.endTime)){and strTime<= '##(:1.endTime)' }"+
			"#if(!empty(:1.maxValue)){and tabVoltage<:1.maxValue} " +
			"#if(!empty(:1.min_early)){and tabVoltage>:1.min_early} " +
			"#if(!empty(:1.max_early)){and tabVoltage<:1.max_early} " +
			") as tabpoll where  sourcemidtabindustrialdrainage.syTime=tabpoll.maxSytime and sourcemidtabindustrialdrainage.rid=tabpoll.rid1")
	 public List<Pollutant>  selectByCriteria_volAPI(Alarm alarm,Pager pager);
