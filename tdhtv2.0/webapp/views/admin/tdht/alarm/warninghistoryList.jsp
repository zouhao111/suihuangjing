...
<body  class="body_overflow">
	...
	<c:choose>
		...
		<input id="select" type="hidden" value="${waterType}" >
			...
			<form action="../alarm/warninghistoryList?rid=${rid}&type=1 " id="myform" name="myform" method="post">
		...
		<input id="select" type="hidden" value="${waterType}" >
			<div style="background-color:#ffffff;width:100%;height:5px;"></div>
			<form action="../alarm/warninghistoryList?rid=${rid}&type=3 " id="myform" name="myform" method="post">
	  ...
	  <c:forEach items="${alarm_types}"  var="bean">
		...
		<input id="select" type="hidden" value="${waterType}" >
			<div style="background-color:#ffffff;width:100%;height:5px;"></div>
			<form action="../alarm/warninghistoryList?rid=${rid}&type=6" id="myform" name="myform" method="post">
	  ...
	  <c:forEach items="${alarm_types}"  var="bean">
		...
		<input id="select" type="hidden" value="${waterType}" >
			<div style="background-color:#ffffff;width:100%;height:5px;"></div>
			<form action="../alarm/warninghistoryList?rid=${rid}&type=5 " id="myform" name="myform" method="post">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table-list" id="table" style="border-collapse:collapse;" >
		...
		<c:forEach items="${historys}" var="bean">
			<tr   class="tb_font15">
				...
				<td align="center" width="100"><fmt:formatNumber type='number' value='${bean.max_early}' /></td>
				<c:choose>
				<c:when test="${bean.min_early == null}">
				<td align="center" width="100">-</td>
				</c:when>
				<c:when test="${bean.min_early != null}">
				<td width="100" align="center"><fmt:formatNumber type='number' value='${bean.min_early}' /></td>
				</c:when>
				</c:choose>
				<td align="center" width="100" style="background-color: yellow;"><fmt:formatNumber type='number' value='${bean.realValue }' /></td>
			</tr>
		</c:forEach>
		</tbody>
	</table>
	...
	<div id="pagelist"  >
		...
		<ul style="list-style:none; ">
			...
			export="currentPageNumber=pageNumber" maxPageItems="${pager.pageSize }" maxIndexPages="5" url="../alarm/warninghistoryList" >
			<!-- 分页带查询条件 -->
