...
<script type="text/javascript">
	...
	//查询
	function findReport(){
		document.myform.action = "../alarm/historyList";
...
<body class="body_overflow">
	...
	<c:choose>
		<c:when test="${type==1}">
		<p class="f14style" ><span class="f14 text-title">${viewCompanyconfig.companyName}历史日报警</span></p>
		<input id="select" type="hidden" value="${waterType}" >
		
		<div style="background-color:#ffffff;width:100%;height:5px;"></div>
			<form action="../alarm/historyList?rid=${rid}&type=1 " id="myform" name="myform" method="post">
		   ...
		   <c:forEach items="${alarm_types}"  var="bean">
		   <option value ="${bean.alarm_type}">${bean.type}</option>
		   </c:forEach>
		      </select> 
		...
		<c:when test="${type==3}">
		<p  class="f14style" ><span class="f14 text-title">${viewCompanyconfig.companyName}历史月报警</span></p>
		<input id="select" type="hidden" value="${waterType}" >
			<div style="background-color:#ffffff;width:100%;height:5px;"></div>
			<form action="../alarm/historyList?rid=${rid}&type=3 " id="myform" name="myform" method="post">
	  ...
	  <c:forEach items="${alarm_types}"  var="bean">
		...
		<c:when test="${type==6}">
		<p  class="f14style" ><span class="f14 text-title">${viewCompanyconfig.companyName}历史年报警</span></p>
		<input id="select" type="hidden" value="${waterType}" >
			<div style="background-color:#ffffff;width:100%;height:5px;"></div>
			<form action="../alarm/historyList?rid=${rid}&type=6" id="myform" name="myform" method="post">
	   <span class="heard_font14">选择年份： </span><input id="dateType" name="dateTypes" value="${dateTypes}"  class="laydate-icon" onclick="laydate({istime: true, format: 'YYYY'})">
			...
			公司编号：<input type="text"  id="" name="rid"  value=" ${rid}" >--%>
	  <span class="heard_font14">污染物类型：</span>	<select id="waterType"name="waterType">
	   <!-- <option value ="PH">PH</option><option value ="COD">COD</option><option value="mlss">固体悬浮物</option><option value="cr6">六价铬</option><option value="pho">总磷</option><option value="ztraffic">总流量</option><option value="vol">电压</option>
	    -->
	    <c:forEach items="${alarm_types}"  var="bean">
		   <option value ="${bean.alarm_type}">${bean.type}</option>
		   </c:forEach>
		...
		<c:when test="${type==5}">
		<p  class="f14style" ><span class="f14 text-title">${viewCompanyconfig.companyName}历史自定义报警</span></p>
		<input id="select" type="hidden" value="${waterType}" >
			<div style="background-color:#ffffff;width:100%;height:5px;"></div>
			<form action="../alarm/historyList?rid=${rid}&type=5 " id="myform" name="myform" method="post">
		<span class="heard_font14">开始时间：</span><input id="startingTime" name="startingTime" value="${sTime}"  class="laydate-icon" onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})">
	     <span class="heard_font14">结束时间：</span><input id="endTime" name="endTime" value="${eTime}"  class="laydate-icon" onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})">
		  ...
		  <span class="heard_font14">污染物类型：</span>	<select id="waterType"name="waterType"> 
<!-- 		  <option value ="PH">PH</option><option value ="COD">COD</option><option value="mlss">固体悬浮物</option><option value="cr6">六价铬</option><option value="pho">总磷</option><option value="ztraffic">总流量</option><option value="vol">电压</option></select>  -->
	...
	<div class="table2excel bluestrip" style="min-height:350px;">
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table-list" id="table" style="border-collapse:collapse;" >
			<thead>
					<tr  class="tb_heard_font16">
						<th width="120" align="center">报警时间</div></th>
						<th width="120" align="center">监测单元名称</div></th>
						<th width="120" align="center">监测单元编号</div></th>
						<th width="120" align="center">报警类型</div></th>
						<th width="100" align="center">报警值上限</div></th>
						<th width="100" align="center">报警值下限</div></th>
						<th width="100" align="center">报警值</div></th>
					</tr>
			</thead>
		<tbody>
		<c:forEach items="${historys}" var="bean">
			<tr   class="tb_font15">
				...
				<td align="center" width="120"  style="font-family:Microsoft YaHei;">${bean.companyName}</td>
				<td align="center" width="120">${bean.rid}</td>
				<td align="center" width="120" style="font-family:Microsoft YaHei;">${bean.alarmtype}</td>
				<td align="center" width="100"><fmt:formatNumber type='number' value='${bean.maxValue}'/></td>
				<c:choose>
				...
				<td align="center" width="100" style="background-color: red;"><fmt:formatNumber type='number' value='${bean.realValue}' /></td>
			</tr>
		</c:forEach>
		</tbody>
	</table>
