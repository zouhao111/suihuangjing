...
<input id="webUrls" type="hidden" value="${webUrls}" >
    <c:choose>
	  <c:when test="${type==1}">
	  <p class="f14style" ><span id="com_name"class="f14 text-title">${companyName}流量汇总系数日曲线</span></p>
	  ...
	  <c:when test="${type==2}">
	     <p class="f14style" ><span id="com_name"class="f14 text-title">${companyName}流量汇总系数周曲线</span></p>
	     <form id="myform" name="myform" method="post" style="text-align: left;margin-top: 5px;">
	     选择周：<input id="startingTime" name="dateTypes" value="${dateTypes}" class="laydate-icon" onclick="laydate({istime: true, format: 'YYYY-MM'})">
	 ...
	 <c:when test="${type==3}">
	  <p class="f14style" ><span id="com_name"class="f14 text-title">${companyName}流量汇总系数月曲线</span></p>
		  <form id="myform" name="myform" method="post" style="text-align: left;margin-top: 5px;">
	...
	<c:when test="${type==6}">
		<p class="f14style" ><span id="com_name"class="f14 text-title">${companyName}流量汇总系数年曲线</span></p>
		<form id="myform" name="myform" method="post" style="text-align: left;margin-top: 5px;">
	...
	<c:otherwise>
		<p class="f14style" ><span id="com_name"class="f14 text-title">${companyName}流量汇总系数自定义曲线</span></p>
		  <form id="myform" name="myform" method="post"  style="text-align: left;margin-top: 5px;">
 ...
 <!-- 为ECharts准备一个具备大小（宽高）的Dom -->
    ...
    function findReport(){
		...
		document.myform.action = "../curveReport/curveReportList2?form=5&type=1&rid="+rid;
		document.myform.submit();
	}
	 function findReport2(){
		$.fn.jqLoading({ height: 100, width: 240, text: "正在加载中，请耐心等待...." });
		document.myform.action = "../curveReport/curveReportList2?form=5&type=2&rid="+rid;
		document.myform.submit();
	}
	 function findReport3(){
		$.fn.jqLoading({ height: 100, width: 240, text: "正在加载中，请耐心等待...." });
		document.myform.action = "../curveReport/curveReportList2?form=5&type=3&rid="+rid;
		document.myform.submit();
	}
	 function findReport4(){
		$.fn.jqLoading({ height: 100, width: 240, text: "正在加载中，请耐心等待...." });
		document.myform.action = "../curveReport/curveReportList2?form=5&type=6&rid="+rid;
		document.myform.submit();
	}
	 function findReport5(){
		$.fn.jqLoading({ height: 100, width: 240, text: "正在加载中，请耐心等待...." });
		document.myform.action = "../curveReport/curveReportList2?form=5&type=5&rid="+rid;
		document.myform.submit();
	}
	...
	//清空   
	 ...
	 var data;
    if(rid==75501330004){
    data=['工业排水','工业给水','损耗限值','损耗值','不明工业排水']
    	datas={'工业排水':true,'工业给水':true,'生活给水':false,'损耗限值':true,'损耗值':true,'损耗值':true}
    	 LSName='生活给水';
        }else if(rid==75501330025){
       data=['工业排水','工业给水','工业给水2','损耗限值','损耗值','不明工业排水']
        	datas= {'工业排水':true,'工业给水':true,'工业给水2':true,'损耗限值':true,'损耗值':true,'损耗值':true}
        	 LSName='工业给水2';
        }else{
         data=['工业排水','工业给水','生活给水','损耗限值','损耗值','不明工业排水']
        	datas= {'工业排水':true,'工业给水':true,'生活给水':true,'损耗限值':true,'损耗值':true,'损耗值':true}
        	 LSName='生活给水';
        }
    </script>
