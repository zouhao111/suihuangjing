...
<input id="webUrls" type="hidden" value="${webUrls}" >
     <c:choose>
	    <c:when test="${type==1}">
	    <p class="f14style" ><span id="com_name"class="f14 text-title">${companyName}工业给水日曲线</span></p>
	    ...
	    <c:when test="${type==2}">
	    <p class="f14style" ><span id="com_name"class="f14 text-title">${companyName}工业给水周曲线</span></p>
	       <form id="myform" name="myform" method="post" style="text-align: left;margin-top: 5px;">
	  ...
	  <c:when test="${type==3}">
	  	<p class="f14style" ><span id="com_name"class="f14 text-title">${companyName}工业给水月曲线</span></p>
	       <form id="myform" name="myform" method="post" style="text-align: left;margin-top: 5px;">
	  ...
	  <c:when test="${type==6}">
	    <p class="f14style" ><span id="com_name"class="f14 text-title">${companyName}工业给水年曲线</span></p>
	    <form id="myform" name="myform" method="post" style="text-align: left;margin-top: 5px;">
	  ...
	  <c:otherwise>
		  <p class="f14style" ><span id="com_name"class="f14 text-title">${companyName}工业给水自定义曲线</span></p>
 ...
 <!-- 为ECharts准备一个具备大小（宽高）的Dom -->
    ...
    function findReport(){
		...
		document.myform.action = "../curveReport/curveReportList?form=2&type=1&rid="+rid;
		document.myform.submit();
	}
	 function findReport2(){
		$.fn.jqLoading({ height: 100, width: 240, text: "正在加载中，请耐心等待...." });
		document.myform.action = "../curveReport/curveReportList?form=2&type=2&rid="+rid;
		document.myform.submit();
	}
	 function findReport3(){
		$.fn.jqLoading({ height: 100, width: 240, text: "正在加载中，请耐心等待...." });
		document.myform.action = "../curveReport/curveReportList?form=2&type=3&rid="+rid;
		document.myform.submit();
	}
	 function findReport4(){
		$.fn.jqLoading({ height: 100, width: 240, text: "正在加载中，请耐心等待...." });
		document.myform.action = "../curveReport/curveReportList?form=2&type=6&rid="+rid;
		document.myform.submit();
	}
	 function findReport5(){
		$.fn.jqLoading({ height: 100, width: 240, text: "正在加载中，请耐心等待...." });
		document.myform.action = "../curveReport/curveReportList?form=2&type=5&rid="+rid;
		document.myform.submit();
	}
