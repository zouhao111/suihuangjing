...
<body>
<div class="title-bar"><strong class="f14 text-title">编辑模块</strong> | <a href="#" onclick="back()">返回</a></div>
<form id="myform" name="myform" method="post">
    ...
    <table id="user-base" border="0" cellspacing="2" cellpadding="3" class="table-form table-form-f12" style="margin-top:20px;">
        <tr>
            ...
            <td><input id="fullName" name="fullName" type="text" class="text" value="${sysMenu.fullName }" style="width:250px" /></td>
        </tr>
        <tr>
            <th align="right"><label for="name">模块编号：</label></th>
            <td><input id="code" name="code" type="text" class="text"  value="${sysMenu.code }" style="width:250px" /></td>
        </tr>
        <tr>
            <th align="right"><label for="name">上级模块：</label></th>
            <td>
            	<input id="menuName" type="text" readonly value="${sysMenu.parentName }" style="width:250px;" onclick="showMenu();"/>
            	<input id="parentId" name="parentId" type="hidden" value="${sysMenu.parentId }">
            </td>
        </tr>
        ...
        <tr>
            ...
            <td>
            	<select name="target" style="width:250px;">
            		<option value="Click" <c:if test="${sysMenu.target == 'Click' }"> selected="selected" </c:if> >Click</option>
            		<option value="Iframe" <c:if test="${sysMenu.target == 'Iframe' }"> selected="selected" </c:if>>Iframe</option>
            	</select>
            </td>
        </tr>
        <tr>
            <th align="right"><label for="name">Web链接地址：</label></th>
            <td><input id="navigateUrl" name="navigateUrl" type="text" class="text"  value="${sysMenu.navigateUrl }" style="width:250px" /></td>
        </tr>
        <tr>
            <th align="right"><label for="name">显示顺序[*]：</label></th>
            <td><input id="sortCode" name="sortCode" type="text" class="text"  value="${sysMenu.sortCode }" style="width:250px" /></td>
        </tr>
        <tr>
            <th align="right"><label for="name">选项[*]：</label></th>
            <td>
            	<input type="checkbox" name="enabled" value="1" <c:if test="${sysMenu.enabled == 1 }"> checked="checked" </c:if>>有效
            </td>
        </tr>
        <tr>
            <th align="right"><label for="name">描述：</label></th>
            <td>
            	<textarea rows="3" cols="30" id="description" name="description">${sysMenu.description }</textarea>
            </td>
        </tr>
    </table>
    <div class="tool-btm" >
