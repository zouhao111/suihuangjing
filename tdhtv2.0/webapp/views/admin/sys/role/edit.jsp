...
<body>
<div class="title-bar"><strong class="f14 text-title">编辑角色</strong> | <a href="#" onclick="back()">返回</a></div>
<form id="myform" name="myform" method="post">
    ...
    <table id="user-base" border="0" cellspacing="2" cellpadding="3" class="table-form table-form-f12" style="margin-top:20px;">
        <tr>
            ...
            <td><input id="code" name="code" type="text" class="text" value="${sysRoles.code }" style="width:250px" /></td>
        </tr>
        <tr>
            <th align="right"><label for="name">角色名称[*]：</label></th>
            <td><input id="fullName" name="fullName" type="text" class="text" value="${sysRoles.fullName }" style="width:250px" /></td>
        </tr>
        <tr>
            <th align="right"><label for="name">角色分类[*]：</label></th>
            <td>
            	<select id="category" name="category" style="width:250px;">
            		<option value="1" <c:if test="${sysRoles.category == 1 }"> selected="selected" </c:if> >系统角色</option>
            		<option value="2" <c:if test="${sysRoles.category == 2 }"> selected="selected" </c:if> >业务角色</option>
            	</select>
            </td>
        </tr>
        <tr>
            ...
            <td>
            	<input id="enabled" name="enabled" type="checkbox"  value="1" <c:if test="${sysRoles.enabled == 1 }"> checked="checked" </c:if>  >有效
            </td>
        </tr>
        <tr>
            <th align="right"><label for="name">描述：</label></th>
            <td><textarea id="description" name="description" rows="3" cols="30" >${sysRoles.description }</textarea></td>
        </tr>
    </table>
    <div class="tool-btm" >
    	<div class="toolbar">
